...
describe('pet-store', () => {
  ...
  describe('updatePet', () => {
    ...
    it('should not affect other pets', () => {
      const pet2 = { ...mockPet, id: '2', name: 'Max' }
      usePetStore.getState().addPet(mockPet)
      usePetStore.getState().addPet(pet2)

      usePetStore.getState().updatePet({ ...mockPet, name: 'Buddy Jr.' })

      expect(usePetStore.getState().pets[1].name).toBe('Max')
    })
  })
  ...
  describe('removePet', () => {
    ...
    it('should not affect other pets', () => {
      ...
      expect(usePetStore.getState().pets).toHaveLength(1)
      expect(usePetStore.getState().pets[0].id).toBe('2')
    })
  })
